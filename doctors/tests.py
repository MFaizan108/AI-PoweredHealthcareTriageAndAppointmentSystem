...
class DoctorViewSetTests(APITestCase):
    def setUp(self):
        ...
        self.admin = User.objects.create_user(
            username="doc_admin", email="doc_admin@example.com", password="x", role=User.Role.ADMIN
        )
        self.patient = User.objects.create_user(
            username="doc_patient", email="doc_patient@example.com", password="x", role=User.Role.PATIENT
        )
...
class DoctorAvailabilityTests(APITestCase):
    def setUp(self):
        self.doctor_user = User.objects.create_user(
            username="avail_doc",
            email="avail_doc@example.com",
            password="x",
            role=User.Role.DOCTOR,
        )
        self.doctor = Doctor.objects.filter(user=self.doctor_user).first()
        self.admin = User.objects.create_user(
            username="avail_admin", email="avail_admin@example.com", password="x", role=User.Role.ADMIN
        )
    ...
    def test_filter_availability_by_doctor(self):
        DoctorAvailability.objects.create(
            doctor=self.doctor, weekday=1, start_time=datetime.time(9, 0), end_time=datetime.time(11, 0)
        )
        self.client.force_authenticate(self.admin)
...
class DoctorAvailabilityCacheTests(APITestCase):
    def setUp(self):
        cache.clear()
        self.doctor_user = User.objects.create_user(
            username="cache_doc", email="cache_doc@example.com", password="x", role=User.Role.DOCTOR
        )
        self.doctor = Doctor.objects.filter(user=self.doctor_user).first()
